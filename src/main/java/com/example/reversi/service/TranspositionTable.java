...
public class TranspositionTable {
    ...
    public Entry get(String key) { return table.get(key); }
    public void put(String key, int depth, int value) { table.put(key, new Entry(depth, value)); }

    public String key(int[][] cells, int player, int depth) {
        StringBuilder sb = new StringBuilder(70);
        for (int[] row : cells) for (int v : row) sb.append(v);
