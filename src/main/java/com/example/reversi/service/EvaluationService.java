...
public class EvaluationService {
    ...
    public int evaluate(int[][] cells, boolean ultimate) {
        if (isTerminal(cells)) return terminalScore(cells);
        int score = 0;
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                if (cells[r][c] == Board.WHITE) score += WEIGHTS[r][c];
                if (cells[r][c] == Board.BLACK) score -= WEIGHTS[r][c];
            }
        }
        ...
        int mobility = boardService.getLegalMoves(cells, Board.WHITE).size() - boardService.getLegalMoves(cells, Board.BLACK).size();
    ...
    private int dangerScore(int[][] cells) {
        ...
        for (int[] p : xSquares) {
            if (cells[p[0]][p[1]] == Board.WHITE) score -= 120;
            if (cells[p[0]][p[1]] == Board.BLACK) score += 120;
        }
        for (int[] p : cSquares) {
            if (cells[p[0]][p[1]] == Board.WHITE) score -= 80;
            if (cells[p[0]][p[1]] == Board.BLACK) score += 80;
        }
        return score;
    }
